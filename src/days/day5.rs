...
fn parse_stack_text(stack_str: &str) -> Result<Vec<VecDeque<char>>, Box<dyn Error>> {
    ...
    let stack_str_idx = lines.last().map(|stack_nums| {
        stack_nums
            .trim_end()
            .chars()
            .enumerate()
...
pub fn crate_mover_9000(fname: &str) -> Result<Vec<char>, Box<dyn Error>> {
    ...
    if let (Some(stack), Some(instructions)) = (stack_instructions.get(0), stack_instructions.get(1)) {
        ...
        for line in instructions.lines() {
            ...
            if let Some(cap) = RGX_INSTRUCTIONS.captures(line) {
                if let (Some(n_crates_mtch), Some(from_stack_mtch), Some(to_stack_mtch)) = (cap.get(1), cap.get(2), cap.get(3)) {
                    ...
                    for _ in 0..n_crates {
                        ...
                        if let (Some(stack_to), Some(moved_item)) = (stacks.get_mut(to_stack_idx - 1), item) {
        ...
        // println!("{:#?}", stacks);
        let last_items = stacks.iter_mut().map(|stack| stack.pop_back().unwrap_or(' ')).collect_vec();
        Ok(last_items)
    } else {
